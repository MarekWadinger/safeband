...
def run_split(
    ...
    # --- TUNE (on the tuning split; capped under --subset) ---
    tune_limit = max(2, subset) if subset is not None else None
    tuning_series = prepare_tuning_series(split, tune_limit, max_tuning_len)
    ...
    # --- EVALUATE (on the eva split) ---
    files = read_split(split, "Eva")
    if subset is not None:
        files = files[:subset]
    logger.info("Evaluating %d %s-Eva series", len(files), split)
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.info("Wrote %d rows to %s", len(results), args.out)
